...
VERSION="1.0.0-1"
SHARDS={
  "28":{"source":"sources/functional-shards/shard28_wasm_wasi.py","output":"staging/shard-28","expected":67,"label":"WASM/WASI 0.3/Component Model"},
  "29":{"source":"sources/functional-shards/shard29_oci_supplychain.py","output":"staging/shard-29","expected":67,"label":"OCI/SBOM/attestation"},
  "30":{"source":"sources/functional-shards/shard30_mcp_models.py","output":"staging/shard-30","expected":66,"label":"MCP 2026/model artifacts"},
}
...
Description: {label} utility ({pkg})
 Functional OceanStudio shard {shard} utility. The installation prefix remains
 {PREFIX}; package-name prefixes only identify the tool family.
"""
...
def existing_names():
    ...
    for path in paths:
        ...
        if path.startswith(("staging/shard-28/","staging/shard-29/","staging/shard-30/")):continue
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--shards",default="28,29,30");a=ap.parse_args()
    nums=[x.strip() for x in a.shards.split(",") if x.strip()]
    existing=existing_names();built=[];seen=set(existing)
    for num in nums:
